...
from .Ui import Ui
from .helper import thread_sleep
...
class Feeder:
    def __init__(self):
        ...
        self.count = 0
        self.key = "i"
        self.tournamentStartTime: datetime = datetime.strptime(config.tournamentStartTime, "%d.%m.%y %H:%M:%S")
        self.wheel = ["|", '/', "-", "\\"]
    ...
    # def current_scheduled_state(self) -> TournamentState:
    #     now = datetime.now()
    #
    ...
    def feed(self):
        while self.running:
            ...
            self.ui.main_window.addstr(2, 20, "time: " + datetime.now().strftime("%H:%M:%S"))
            self.ui.main_window.addstr(3, 1,
                                       "tournament starts @:     " + self.tournamentStartTime.strftime("%H:%M:%S"))
            ...
            for k in self.gt.ps.jingleQueue:
                ...
                self.ui.main_window.addstr(line, 2, k.strftime("%H:%M:%S") + " " + str(self.gt.ps.jingleQueue[k][0]))
                nextfreeline += 1
            ...
            self.ui.match_window.addstr(4, 1, "Remaining Time :  " + str(round(
                (self.gt.matchEndTime - datetime.now()).total_seconds())) + "s")
            ...
            self.ui.break_window.addstr(4, 1, "Remaining Time :  " + str(round(
                (self.gt.breakEndTime - datetime.now()).total_seconds())) + "s")
            ...
            if self.gt.tournamentState == TournamentState.NotStarted:
                if self.tournamentStartTime - config.breakLength > datetime.now():
